...
/* Raw in, clean out: /vive_vr/raw/<x> -> /vive_vr/<x>.
 *
...
#include <vive_vr_ros2/msg/eye_gaze.hpp>
...
{
  public:
    explicit InputNode(const rclcpp::NodeOptions &options) : Node("vive_input", options)
    {
        world_frame_  = declare_parameter<std::string>("world_frame", "world");
        origin_frame_ = declare_parameter<std::string>("origin_frame", "vr_origin");
        raw_ns_       = declare_parameter<std::string>("raw_ns", "/vive_vr/raw");
        ...
        if (declare_parameter<bool>("publish_gaze", true)) {
            gaze_pub_ = create_publisher<vive_vr_ros2::msg::EyeGaze>(out_ns_ + "/gaze",
                                                           rclcpp::SensorDataQoS());
            gaze_sub_ = create_subscription<vive_vr_ros2::msg::EyeGaze>(
              raw_ns_ + "/gaze", rclcpp::SensorDataQoS(),
              [this](vive_vr_ros2::msg::EyeGaze::SharedPtr msg) { on_gaze(*msg); });
        }
        ...
        const auto pc_time_topic =
          declare_parameter<std::string>("pc_time_topic", "/vive_vr/pc_time");
  ...
  private:
    ...
    void on_gaze(const vive_vr_ros2::msg::EyeGaze &raw)
    {
        vive_vr_ros2::msg::EyeGaze out = raw;
        const bool client_stamped = raw.header.stamp.sec || raw.header.stamp.nanosec;
    ...
    std::unordered_map<std::string, rclcpp::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr>
                                                                                     joints_pubs_;
    rclcpp::Publisher<vive_vr_ros2::msg::EyeGaze>::SharedPtr                                      gaze_pub_;
    rclcpp::Subscription<vive_vr_ros2::msg::EyeGaze>::SharedPtr                                   gaze_sub_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(vive_vr_ros2::InputNode)
