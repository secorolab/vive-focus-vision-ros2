...
/** The arm names under `teleop.<arm>.<key>`. NodeOptions holds only what a caller set in code,
 *  so a --params-file is visible on the parameters interface instead. */
...
class TeleopNode : public rclcpp::Node
{
  ...
  private:
    struct Arm
    {
         ...
         * tool frame puts +Z along the approach, so identity points the gripper backwards. */
        tf2::Transform tool_R = tf2::Transform::getIdentity();

        // An axis rather than the button, so a partial grip survives the trip.
        int gripper_axis = 2;
    ...
    void add_arm(const std::string &name)
    {
        ...
        // Latched: a late consumer must learn the clutch is open, not wait for a press.
        ...
        // Subscribed and unused, so the topic contract does not change when it is needed.
        if (!ee_topic.empty()) {
    ...
    void on_joy(Arm &arm, const sensor_msgs::msg::Joy &msg)
    {
        ...
        // A fresh reference per press: re-clutching recovers reach the way lifting a mouse does.
        arm.ref      = arm.pose;
        arm.clutched = true;
...
RCLCPP_COMPONENTS_REGISTER_NODE(vive_vr_ros2::TeleopNode)
