...
#
# Source to headset in one command, from any machine.
#
#   ./tools/build_apk.sh                    build the APK
#   ./tools/build_apk.sh --setup            rewrite the scene and player settings first
#   ./tools/build_apk.sh --install          build, then install and launch on the headset
...
# optional because the client discovers the PC when the configured address fails.
...
if ! ls "$project"/vendor/*.tgz >/dev/null 2>&1; then
    echo "VIVE plugin missing; running tools/fetch_vive_plugin.sh"
    "$here/tools/fetch_vive_plugin.sh"
fi

# One instance per project: a running editor makes these invocations fail on the lockfile.
...
# A rename makes a different application, so an older install is not replaced but joined.
