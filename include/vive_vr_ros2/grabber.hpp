...
struct GrabConf
{
    std::string topic_ns    = "/vive_vr"; // -> <topic_ns>/<hand>/{pose,joy,joints}
    ...
    // Hand velocity comes from poses arriving in bursts over Wi-Fi; unbounded, the grab jitters.
 ...
 *
 *   vive_vr_ros2::Grabber grab(*node, model, conf);
 *   while (running) {
 ...
 * put, and pushing a robot link fights its actuators.
 */
class Grabber
{
  public:
